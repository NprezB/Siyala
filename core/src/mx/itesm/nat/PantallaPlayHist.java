...
public class PantallaPlayHist extends Pantalla {
    ...
    private mx.itesm.nat.Siyala juego;
    ...
    private mx.itesm.nat.Objeto botonPausa;
    private Texture texturaPausa;
    private boolean pausa;
    private Texture texturaContinuar;
    private Texture texturaMenu;
    private mx.itesm.nat.Objeto botonContinuar;
    private mx.itesm.nat.Objeto botonMenu;
    private Sprite spriteFondo;
    private Texture texturaFondo;

    //Pantalla secundaria fin
    private Sprite spriteGameOv;
    private mx.itesm.nat.Objeto botonPlay;
    ...
    public PantallaPlayHist(mx.itesm.nat.Siyala juego) {
        this.juego = juego;
    ...
    public void show() {
        ...
        botonPausa=new mx.itesm.nat.Objeto(texturaPausa,camara.position.x+320,camara.position.y+texturaPausa.getHeight());

        //Boton Continuar
        texturaContinuar=manager.get("Botones/Continue1.png");
        botonContinuar=new mx.itesm.nat.Objeto(texturaContinuar,camara.position.x, 3*(camara.position.y/2)+texturaContinuar.getHeight());

        //Boton Menu
        texturaMenu=manager.get("Botones/BotonExit1.png");
        botonMenu=new mx.itesm.nat.Objeto(texturaMenu,camara.position.x,camara.position.y/2);
        //Fondo pausa
        ...
        botonPlay=new mx.itesm.nat.Objeto(texturaPlay,320,0);
    ...
    private void ponerEfectos(){
        if(siyala.getEstadoMovimiento()== Personaje.EstadoMovimiento.MOV_DERECHA && mx.itesm.nat.Setts.getefect() ){
            effCamina.play();
        }
        else {
            effCamina.pause();
            if((siyala.getEstadoMovimiento()== Personaje.EstadoMovimiento.SUBIENDO ||
                    siyala.getEstadoMovimiento()== Personaje.EstadoMovimiento.BAJANDO) && mx.itesm.nat.Setts.getefect()){
                effBrinca.play();
            }
            else{
                effBrinca.pause();
    ...
    public void nextLevel(){
        mx.itesm.nat.Setts.actualizarNiveles(2);
