...
public class PantallaCarga extends Pantalla {
    ...
    private mx.itesm.nat.Pantallas sigPantalla;
    private int porcent;
    private mx.itesm.nat.Texto texto;

    private Texture textCargando;

    public PantallaCarga(mx.itesm.nat.Siyala juego, mx.itesm.nat.Pantallas sigPant){
        this.juego=juego;
    ...
    public void show() {
        ...
        texto=new mx.itesm.nat.Texto("fuente.fnt");
    ...
    private void actualizarCargaRecursos() {
        if(manager.update()) {
            switch (sigPantalla) {
                case PLAYHIST3:
                    juego.setScreen(new mx.itesm.nat.PantallaPlayHist3(juego));
                    break;
                case PLAYHIST:
                    juego.setScreen(new mx.itesm.nat.PantallaPlayHist(juego));
                    break;
                case MENU:
                   juego.setScreen(new mx.itesm.nat.PantallaMenu(juego));
                    break;
                case CREDITOS:
                    juego.setScreen(new mx.itesm.nat.PantallaCreditos(juego));
                    break;
                case SETTS:
                    juego.setScreen(new mx.itesm.nat.PantallaSettings(juego));
                    break;
                case INSTRUCCIONES:
                    juego.setScreen(new mx.itesm.nat.PantallaInstrucciones(juego));
                    break;
                case PLAYHIST2:
                    juego.setScreen(new mx.itesm.nat.PantallaPlayHist2(juego));
                    break;
                case PLAYSURV:
                    juego.setScreen(new mx.itesm.nat.PantallaPlaySurv(juego));
                    break;
                case NIVELES:
                    juego.setScreen(new mx.itesm.nat.PantallaNiveles(juego));
                    break;
            }
        }
